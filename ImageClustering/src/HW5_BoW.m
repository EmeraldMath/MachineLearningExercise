...
% Last modified: 16-Oct-2018    
    ...
    methods (Static)
        function main()
            ...
            fprintf('Model with defulat parameters\n');
 ...
 %-------------------------------           
            ...
            for i = -5:2:15
                ...
                for j = -5:2:15
                    ...
                    fprintf('C = %f , gamma = %f \n', pc, py);
                    model = svmtrain(trLbs, trD', option);
                end
            end
%             
%             t_data = tstD';
...
%             norm_t_data(isnan(norm_t_data)) = 0;
            ...
            for i = -5:2:15
                ...
                for j = -5:2:15
                   ...
                   %0.0001:0.002:0.001
                   %0.001:0.002:0.01
                   %C = 32.000000 , gamma = 0.007812 
                   %Cross Validation Accuracy = 89.6455%
...
%-------------------------------------------------            
            ...
            [trainK, testK] = Xkernel.cmpExpX2Kernel(norm_data, norm_t_data, 0.007812);
            model = svmtrain(trLbs, trainK, '-s 0 -c 32 -t 4 -q');
            [tn,td] = size(norm_t_data);
