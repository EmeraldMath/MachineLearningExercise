...
for i = 1:1:row
    label = (v==y(i))';
    J = J -log(h(i,:))*label-log(1-h(i,:))*(1-label);
end
J = 1/m*sum(J);

J = J + lambda/(2*m)*(sum(Theta1(:,2:end).*Theta1(:,2:end),'all') + sum(Theta2(:,2:end).*Theta2(:,2:end),'all'));
...
% ------------------------Part 3:
r1 = size(Theta1_grad,1);
c1 = size(Theta1_grad,2);
r2 = size(Theta2_grad,1);
c2 = size(Theta2_grad,2);
